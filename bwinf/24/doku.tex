...
Eine Folge von Übungen wird dabei als ASCII-String gespeichert, in dem jedes Zeichen für eine Übung steht. und der an die Funktion \function{countParts} übergeben wird.
...
Die Umwandlung der \type{BigNumber} in einen String wird von der \function{toStr} übernommen, die die umzuwandelte \type{BigNumber} als Parameter entgegen nimmt und den Ergebnisstring als Rückgabewert zurückgibt. (Was könnte man auch sonst mit einem Rückgabewert machen?)\n
...
Man muss dabei nur auf führende Nullen achten, das erstemal, wenn man die in einen String umgewandelten Stellen in den Ergebnisstring kopiert (da sie nicht unbedingt 9 Stellen haben und man sie nach rechts ausrichten muss) und das zweitemal, wenn man alle Stellen der \type{BigNumber} kopiert hat. (dann muss man nämlich die führenden Nullen löschen).

Aufgerufen wird die am Anfang beschriebene Funktion \function{countParts} von der Methode \function{countClick}, die beim Klick auf den Button \component{count} aufgerufen wird.\n
Diese liest den String mit den Übungen entweder aus dem Editfeld \component{folge} aus, oder aus einer Datei deren Name in dem eben erwähnten Editfeld eingegeben wird. (welcher Fall konkret eintritt hängt von der Markierung des Radiobuttons \component{direct} ab).\n
...
Das Programm kann die Möglichkeiten zusätzlich zum Zählen auch noch auflisten (allerdings natürlich nur bei kurzen Strings), wofür die Methode \function{listClick} zuständig ist.\n
